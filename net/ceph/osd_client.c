					       ...
					       bool use_mempool)
{
	...
	if (num_ops > 1) {
		...
		/*
		 * If this is a multi-op write request, then assume that we'll
		 * need request ops. If it's a multi-op read then assume we'll
		 * need, reply ops. Anything else and call it -EINVAL.
		 */
		if (flags & CEPH_OSD_FLAG_WRITE) {
			num_req_ops = num_ops;
			num_rep_ops = 0;
